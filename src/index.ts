...
const ALCHEMY_API_KEY = process.env.ALCHEMY_API_KEY || undefined; // filter out empty string
...
const provider = new BatchedWebSocketAugmentedAlchemyProvider(NETWORK, ALCHEMY_API_KEY);
const liquity = EthersLiquity.connect(provider);
const poller = new LQTYCirculatingSupplyPoller(liquity, EXCLUDED_ADDRESSES);
